...
export const create = async (
  recipe: Recipe,
): Promise<{ recipe: Recipe; user: User }> => {
  const recipeToPublish = recipe.ingredients
    ? {
        ...recipe,
        ingredients: recipe.ingredients.map((i) =>
          i.fdcItem ? { ...i, fdcItem: i.fdcItem._id } : i,
        ),
      }
    : recipe;
...
export const popularRecipes = async (sortDir: string): Promise<Recipe[]> => {
  const response = await api.post(`${SERVICE_URL}/recipes`, null, {
    params: { sortBy: "popular", sortDir },
  });
  return response.data;
};
